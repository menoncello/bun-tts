...
function testConstructorWithAllProperties() {
  test(`${TestIdGenerator.generateUnit('1.2', 5)} should create error with all properties`, () => {
    const bddComment = BDDTemplateFactory.createGivenWhenThenComment(
      [
        'Error creation parameters are available including code, message, location, and confidence',
        'The MarkdownParseError constructor is ready to create a new error instance',
        'All required error properties need to be properly initialized',
      ],
      [
        'A MarkdownParseError is created with full parameter set',
        'The error constructor processes all provided parameters',
        'Error properties are initialized and formatted appropriately',
      ],
      [
        'All error properties should be correctly set',
        'The error category should be "parsing"',
        'The error code should match the provided code',
        'The message should include both the error message and location',
        'The confidence should be properly stored',
      ]
    );

    const location = { line: 10, column: 5, context: 'Some context' };
...
function testConstructorWithoutLocation() {
  test('should create error without location', () => {
    const error = new MarkdownParseError('PARSE_FAILED', 'Test message');
    expect(error.category).toBe('parsing');
...
function testConstructorClampsConfidence() {
  test('should clamp confidence to valid range', () => {
    const highConfidence = new MarkdownParseError('PARSE_FAILED', 'Test', {
      confidence: 1.5,
    });
    const lowConfidence = new MarkdownParseError('PARSE_FAILED', 'Test', {
      confidence: -0.5,
    });
...
function testConstructorMaintainsStackTrace() {
  test('should maintain proper stack trace', () => {
    const error = new MarkdownParseError('PARSE_FAILED', 'Test');
...
function testCreateInvalidSyntaxError() {
  test(`${TestIdGenerator.generateUnit('1.2', 6)} should create invalid syntax error`, () => {
    const bddComment = BDDTemplateFactory.createGivenWhenThenComment(
      [
        'A factory method for creating invalid syntax errors is available',
        'Error details include a description and location information',
        'The factory method should create a properly formatted error',
      ],
      [
        'The invalidSyntax factory method is called with parameters',
        'A MarkdownParseError instance is created with appropriate properties',
        'The error message is formatted with the provided description',
      ],
      [
        'The error code should be INVALID_SYNTAX',
        'The error message should contain the formatted description',
        'The location should be correctly set on the error',
      ]
    );
...
function testShouldHaveAllRequiredErrorCodes() {
  test('should have all required error codes', () => {
    ...
    expectedCodes.forEach((code) => {
      const error = new MarkdownParseError(code as any, 'Test');

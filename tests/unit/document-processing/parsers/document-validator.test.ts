...
} from '../../../../src/core/document-processing/errors/markdown-parse-error';
import {
...
describe('Document Validator [Priority: CRITICAL]', () => {
  ...
  describe('validateAndPrepareInput', () => {
    ...
    describe('Invalid Input Handling', () => {
      ...
      it('should handle non-Buffer objects with buffer-like properties', () => {
        const fakeBuffer = {
          toString: mock(() => 'fake content'),
        } as any;

        const result = validateAndPrepareInput(fakeBuffer, mockConfig);
        expect(result).toBeInstanceOf(MarkdownParseError);
      });
    });
  ...
  describe('handleParsingError', () => {
    describe('Error Types', () => {
      it('should handle standard Error objects', () => {
        ...
        if (!result.success) {
          ...
          expect(result.error.message).toBe('Parse failed: Parsing failed');
          expect(result.error.cause).toBe(error);
        }
        expect(mockLogger.error).toHaveBeenCalledWith(
          'Markdown parsing failed',
          expect.objectContaining({
            error: 'Parse failed: Parsing failed',
      ...
      it('should handle Error objects with stack traces', () => {
        ...
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.error.message).toBe('Parse failed: Detailed error');
          expect(result.error.cause).toBe(error);
        }
      });
      ...
      it('should handle string errors', () => {
        ...
        if (!result.success) {
          ...
          expect(result.error.message).toBe(
            'Parse failed: String error message'
          );
      ...
      it('should handle number errors', () => {
        ...
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.error.message).toBe('Parse failed: Unknown error');
          expect(result.error.cause).toBeUndefined();
        }
      });
      ...
      it('should handle null/undefined errors', () => {
        ...
        if (!result1.success) {
          expect(result1.error.message).toBe('Parse failed: Unknown error');
        }
        expect(result2.success).toBe(false);
        if (!result2.success) {
          expect(result2.error.message).toBe('Parse failed: Unknown error');
        }
      });
      ...
      it('should handle MarkdownParseError instances', () => {
        ...
        if (!result.success) {
          ...
          expect(result.error.message).toBe(
            'Parse failed: Invalid input type: Invalid markdown (expected: string)'
          );
    ...
    describe('Timing and Logging', () => {
      ...
      it('should log error details correctly', () => {
        ...
        expect(mockLogger.error).toHaveBeenCalledWith(
          'Markdown parsing failed',
          expect.objectContaining({
            error: 'Parse failed: Detailed parsing error',
            duration: expect.any(Number),
          })
        );
      });
    ...
    describe('Result Structure', () => {
      ...
      it('should include error details', () => {
        ...
        if (!result.success) {
          expect(result.error.message).toBe('Parse failed: Test error');
        }
      });
    ...
    describe('Edge Cases', () => {
      ...
      it('should handle very long error messages', () => {
        ...
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.error.message).toBe(`Parse failed: ${longMessage}`);
        }
      });
      ...
      it('should handle errors with no message', () => {
        // Create an error with minimal message to avoid empty string violation
        // This tests the edge case where errors have very short messages
        const error = new Error(' ');
        const startTime = new Date();
  ...
  describe('Integration Tests', () => {
    ...
    it('should maintain error context through the flow', () => {
      ...
      if (!result.success) {
        expect(result.error).toBeInstanceOf(MarkdownParseError);
        expect(result.error.message).toBe('Parse failed: Context error');
        expect(result.error.cause).toBe(error);
      }
    });
  });

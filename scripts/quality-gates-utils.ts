...
import { QualityGateResult } from './quality-gates-validator';
...
export function getSourceFiles(): string[] {
  const srcTsFiles = Array.from(new Bun.Glob('**/*.ts').scanSync({ cwd: 'src' }));
  const srcTsxFiles = Array.from(new Bun.Glob('**/*.tsx').scanSync({ cwd: 'src' }));
  ...
  return [...srcTsFiles, ...srcTsxFiles, ...testsTsFiles, ...testsTsxFiles]
